...
TEST(DepthAICameraTest, BasicTest)
{
  ...
  auto left_subscriber = rclcpp::create_subscription<ImageMsg>(
    *camera_node, "camera/left/image_raw", rclcpp::SensorDataQoS(), [](
      const ImageMsg::SharedPtr a) {
      (void)a;
    });

  auto right_subscriber = rclcpp::create_subscription<ImageMsg>(
    *camera_node, "camera/right/image_raw", rclcpp::SensorDataQoS(), [](
      const ImageMsg::SharedPtr a) {
      (void)a;
    });

  auto color_subscriber = rclcpp::create_subscription<ImageMsg>(
    *camera_node, "camera/color/image_raw", rclcpp::SensorDataQoS(), [](
      const ImageMsg::SharedPtr a) {
      (void)a;
    });

  auto video_subscriber = rclcpp::create_subscription<CompressedImageMsg>(
    *camera_node, "camera/color/video", rclcpp::SensorDataQoS(),
    [](const CompressedImageMsg::SharedPtr a) {(void)a;});
