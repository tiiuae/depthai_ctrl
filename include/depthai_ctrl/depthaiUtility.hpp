...
#define DEPTHAI_ROS_DEBUG_STREAM(loggerName, args) DEPTHAI_ROS_LOG_STREAM( \
    loggerName, \
    dai::ros::LogLevel::DEBUG, \
    false, args)

#define DEPTHAI_ROS_DEBUG_STREAM_ONCE(loggerName, args) DEPTHAI_ROS_LOG_STREAM( \
    loggerName, \
    dai::ros::LogLevel::DEBUG, true, \
    args)

// INFO stream macros on top of ROS logger
#define DEPTHAI_ROS_INFO_STREAM(loggerName, args) DEPTHAI_ROS_LOG_STREAM( \
    loggerName, \
    dai::ros::LogLevel::INFO, \
    false, args)

#define DEPTHAI_ROS_INFO_STREAM_ONCE(loggerName, args) DEPTHAI_ROS_LOG_STREAM( \
    loggerName, \
    dai::ros::LogLevel::INFO, true, \
    args)

// WARN stream macros on top of ROS logger
#define DEPTHAI_ROS_WARN_STREAM(loggerName, args) DEPTHAI_ROS_LOG_STREAM( \
    loggerName, \
    dai::ros::LogLevel::WARN, \
    false, args)

#define DEPTHAI_ROS_WARN_STREAM_ONCE(loggerName, args) DEPTHAI_ROS_LOG_STREAM( \
    loggerName, \
    dai::ros::LogLevel::WARN, true, \
    args)

// ERROR stream macros on top of ROS logger
#define DEPTHAI_ROS_ERROR_STREAM(loggerName, args) DEPTHAI_ROS_LOG_STREAM( \
    loggerName, \
    dai::ros::LogLevel::ERROR, \
    false, args)

#define DEPTHAI_ROS_ERROR_STREAM_ONCE(loggerName, args) DEPTHAI_ROS_LOG_STREAM( \
    loggerName, \
    dai::ros::LogLevel::ERROR, true, \
    args)

// FATAL stream macros on top of ROS logger
#define DEPTHAI_ROS_FATAL_STREAM(loggerName, args) DEPTHAI_ROS_LOG_STREAM( \
    loggerName, \
    dai::ros::LogLevel::FATAL, \
    false, args)

#define DEPTHAI_ROS_FATAL_STREAM_ONCE(loggerName, args) DEPTHAI_ROS_LOG_STREAM( \
    loggerName, \
    dai::ros::LogLevel::FATAL, true, \
    args)
...
inline rclcpp::Time getFrameTime(
  ...
  std::chrono::steady_clock::duration> currTimePoint)
{
